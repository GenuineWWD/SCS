...
def eval_loop(output_file,dataloader,model,tokenizer,generation_config):
    correct = 0
    total = 0
        ...
        for batch in tqdm(dataloader, desc="Running Batch Evaluation"):
            ...
            for i, response in enumerate(responses):
                ...
                if match:
                    ...
                    fallback_matches = re.findall(r"\b([A-Z])\b", response)
                    if fallback_matches:
...
def eval(args):
    ...
    dataset = load_dataset("path/to/m3cot", split='test')
...
def main():
    parser = argparse.ArgumentParser(description="Dynamic image preprocessor with tiling.")
    parser.add_argument('--model_path', type=str,  help="model path")
    parser.add_argument('--batch_size', type=int, default=12, help="Maximum number of tiles.")
    parser.add_argument('--output_file', type=str, default=None, help="Optional path to save tensor output (pt file).")
