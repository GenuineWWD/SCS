...
def consistency_reward_entropy(responses):
    #answer_set = set()
    answer_dict = {}
    total = len(responses)
    for key,value in responses.items():
        parse_answer = extract_answer(value)
        parse_answer = find_first_uppercase(parse_answer)
        if parse_answer in answer_dict:
            answer_dict[parse_answer] += 1 
        else:
            answer_dict[parse_answer] = 1 
        
    entropy = 0

    for count in answer_dict.values():
        p = count/total
        entropy -= p * log2(p)
    
    return entropy * -1   #熵越大一致性越差
...
def get_reward():
    ...
    for q,problem in zip(data["query"],data["prompts"]):
        ...
        consist_r = consistency_reward_entropy(data['cut_response_text_dict'])
